...
dhtmlXGridObject.prototype.enablePaging = function(fl,pageSize,pagesInGrp,parentObj,showRecInfo,recInfoParentObj){this._pgn_parentObj = typeof(parentObj)=="string" ? document.getElementById(parentObj) : parentObj;this._pgn_recInfoParentObj = typeof(recInfoParentObj)=="string" ? document.getElementById(recInfoParentObj) : recInfoParentObj;this.pagingOn = fl;this.showRecInfo = showRecInfo;this.rowsBufferOutSize = parseInt(pageSize);this.currentPage = 1;this.pagesInGroup = parseInt(pagesInGrp);this._init_pgn_events()
 ...
 dhtmlXGridObject.prototype._pgn_link=function(mode,ac,ds){
	...
	//***************** Start firstPage added by Kevin **************//
	if (mode=="firstpages" || mode=="first"){
		 if (this.currentPage==1)
		 	return ds;
		 return '<a onclick=\'this.grid.changePage(0);return false;\'>'+ac+'</a>'
	};
	//***************** End Added by Kevin **************//
 	...
 	//********************Start Last Pages Added by Kevin ********************//
  	if (mode=="lastpages" || mode=="last"){
		 // alert(this.rowsBuffer.length+"   "+this.rowsBufferOutSize+" "+this.currentPage);
		 //alert(this.pagesInGroup);
	 	if (this.rowsBuffer.length/this.rowsBufferOutSize <= this.currentPage )
	 		return ds;
	 	if (this.rowsBuffer.length/(this.rowsBufferOutSize*(mode=="last"?'1':this.pagesInGroup)) <= 1 ) 
	 		return ds;
	 	return '<a onclick=\'this.grid.changePageRelative(500000000);return false;\'>'+ac+'</a>'
 	};
 	//********************** End Added By Kevin ******************//
 	...
 	return ac;
 };dhtmlXGridObject.prototype._pgn_template_active=function(block){var tags=block.getElementsByTagName("A");if (tags)for (var i=0;i < tags.length;i++){tags[i].grid=this;}};dhtmlXGridObject.prototype._pgn_template_compile=function(template){template=template.replace(/\[([^\]]*)\]/g,function(a,b){b=b.split(":");switch (b[0]){
 case "from": 
 ...
 case "prevpages":
 case "nextpages":
 //****************Start lastpages added by Kevin ************************//
 case "lastpages":
 //****************End  added by Kevin ******************************//
...
dhtmlXGridObject.prototype.i18n.paging={results:global_paging_message.results,
	records:global_paging_message.records,
	to:global_paging_message.to,
	page: global_paging_message.page,
	perpage: global_paging_message.perpage,
	first: global_paging_message.first,
	previous: global_paging_message.previous,
	found: global_paging_message.found,
	next: global_paging_message.next,
	last: global_paging_message.last,
	of: global_paging_message.of,
	notfound: global_paging_message.notfound
};
dhtmlXGridObject.prototype.setPagingWTMode = function(navButtons,navLabel,pageSelect,perPageSelect){
	...
	var sfx="";if(tmp=="light" || tmp=="modern" || tmp=="skyblue")sfx = "_"+tmp;
	...
	//alert(this.pagerElAr["pagerCont"].innerHTML+"|"+this.pagerElAr["pagerLine"].innerHTML+"|"+this.pagerElAr["pagerInfo"].innerHTML+"|"+this.pagerElAr["pagerInfoBox"].innerHTML);
	...
	if(this.rowsBuffer.length>0){
		...
		if(page>this.pagesInGroup){
			//***************** Start Kevin**************************************//
			var firstPageCont = document.createElement("DIV");
			var firstPageBox = document.createElement("DIV");
			firstPageCont.className = "dhx_page"+sfx;
			//firstPageBox.style.padding = "0px 2px 0px 2px";
			firstPageBox.innerHTML = "|&lt;"; //|<
			firstPageCont.appendChild(firstPageBox);
			this.pagerElAr["pagerBox"].appendChild(firstPageCont);
			var self = this;
			firstPageCont.pgnum = (Math.ceil(page/this.pagesInGroup)-1)*this.pagesInGroup;
			firstPageCont.onclick = function(){
				self.changePage(-1);
			};
		 ...
		 //alert(this.pagerElAr["pagerBox"].innerHTML);
			for(var i=1;i<=this.pagesInGroup;i++){
				...
				if(pageNumber>Math.ceil(this.rowsBuffer.length/this.rowsBufferOutSize))
			 	...
			 	if(Math.ceil(page/this.pagesInGroup)*this.pagesInGroup<Math.ceil(this.rowsBuffer.length/this.rowsBufferOutSize)){
				 	...
				 	pageCont.appendChild(pageBox);
				 	this.pagerElAr["pagerBox"].appendChild(pageCont);		 	
				 	var self = this;pageCont.pgnum = (Math.ceil(page/this.pagesInGroup)*this.pagesInGroup)+1;
				 	pageCont.onclick = function(){self.changePage(this.pgnum);};
				 	lineWidth +=lineWidthInc;
				 	//******* Start Last Page added by Kevin *********************************//
				 	var pageCont = document.createElement("DIV");
				 	var pageBox = document.createElement("DIV");
				 	pageCont.className = "dhx_page"+sfx;
				 	//pageBox.style.padding = "0px 2px 0px 2px";
				 	pageBox.innerHTML = "&gt;|"; //>|
				 	pageCont.appendChild(pageBox);
				 	this.pagerElAr["pagerBox"].appendChild(pageCont);		 	
				 	var self = this;pageCont.pgnum = (Math.ceil(page/this.pagesInGroup)*this.pagesInGroup)+1;
				 	pageCont.onclick = function(){self.changePage(500000000);};
				 	lineWidth +=lineWidthInc;
				 	//******* End Last Page added by Kevin **********************************//
				 };
				 ...
				 if(this.rowsBuffer.length>0 && this.showRecInfo)
				 	this.pagerElAr["pagerInfoBox"].innerHTML = this.i18n.paging.records+(start+1)+this.i18n.paging.to+end+this.i18n.paging.of+this.rowsBuffer.length;
					 	...
					 	//*** Set cursor of "no record found" section to default. added by Lyn. version date:2011/9/5 ****// 
					 	this.pagerElAr["pagerInfo"].style.cursor = "default";
					 };
		...
		dhtmlXGridObject.prototype._pgn_toolbar = function(page, start, end){
			...
			if (this._WTDef[2]){
				var that=this;this.aToolBar.forEachListOption("pages", function(id){
					...
					if (this._WTDef[1]){
 					...
 					dhtmlXGridObject.prototype._pgn_createToolBar = function(){
	 					...
	 					var self=this;this.aToolBar.attachEvent("onClick",function(val){
		 					val=val.split("_")
