...
/**
 * SVEN: 8/15/2011
 * Fix Grid combo_select can not be hidden under IE6,7,8,9
 * Change: if ((_isOpera&&_OperaRv < 9)||_isKHTML||(_isFF)) to if ((_isOpera&&_OperaRv < 9)||_isKHTML||(_isFF) || _isIE)
 */

/**
 * Andy: 8/24/2011
 * Fix Grid input text can not be selected under IE6,7
 * Change: 'this.entBox.onselectstart=function(){return false' to 'this.entBox.onselectstart=function(){if(_isIE) return event.srcElement.type== "text";'
 */
var globalActiveDHTMLGridObject;String.prototype._dhx_trim=function(){return this.replace(/&nbsp;/g, " ").replace(/(^[ \t]*)|([ \t]*$)/g, "");};function dhtmlxArray(ar){return dhtmlXHeir((ar||new Array()), dhtmlxArray._master);};dhtmlxArray._master={_dhx_find:function(pattern){for (var i = 0;i < this.length;i++){if (pattern == this[i])return i;};return -1;},
 ...
 ||"100%";this.entBox.style.cursor='default';this.entBox.onselectstart=function(){if(_isIE) return event.srcElement.type== "text";
 ...
 //For hide the DropDownList in IE, add  _isIE
 if ((_isOpera&&_OperaRv < 9)||_isKHTML||(_isFF) || _isIE)
 {this._fixHiddenRowsAll(this.hdr, ind, "display", "none","_cellIndexS");};if (this.ftr)this._fixHiddenRowsAll(this.ftr.childNodes[0], ind, "display", "none");this._fixHiddenRowsAll(this.hdr, ind, "whiteSpace", "nowrap","_cellIndexS");if (!this.cellWidthPX.length&&!this.cellWidthPC.length)this.cellWidthPX=[].concat(this.initCellWidth);if (this.cellWidthPX[ind])this.cellWidthPX[ind]=0;if (this.cellWidthPC[ind])this.cellWidthPC[ind]=0;}else {if (this.hdr.rows[0].cells[ind]._oldWidth){var zrow = this.hdr.rows[0].cells[ind];
 //For hide the DropDownList in IE, add  _isIE
