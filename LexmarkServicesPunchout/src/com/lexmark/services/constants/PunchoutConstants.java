package com.lexmark.services.constants;

public class PunchoutConstants {
	/** Constant for PunchoutConstants */
	public static final String CART_SESSION="cartStore";
	/** Constant for PunchoutConstants */
	public static final String CUR_TIME="time";
	/** Constant for PunchoutConstants */
	public static final String SIZE="size";
	/** Constant for PunchoutConstants */
	public static final String PRODUCT_BUNDLE="bundle";
	/** Constant for PunchoutConstants */
	public static final String TOTAL="total";
	/** Constant for PunchoutConstants */
	public static final String ID="id";
	/** Constant for PunchoutConstants */
	public static final String STATUS = "status";
	/** Constant for PunchoutConstants */
	public static final String QUANTITY = "quantity";
	/** Constant for PunchoutConstants */
	public static final String PUNCHOUT_ACCOUNT = "punchoutAccount";
	/** Constant for PunchoutConstants */
	public static final String CART_BUNDLE = "cartBundle";
	/** Constant for PunchoutConstants */
	public static final String BUNDLE_LIST = "bundleList";
	/** Constant for PunchoutConstants */
	public static final String SUCCESS = "success";
	/** Constant for PunchoutConstants */
	public static final String FAILURE = "failure";
	/** Constant for PunchoutConstants */
	public static final String ZERO = "0";
	/** Constant for PunchoutConstants */
	public static final String TOTAL_PRICE = "totalPrice";
	/** Constant for PunchoutConstants */
	public static final String GRID_DATA = "gridData";
	/** Constant for PunchoutConstants */
	public static final String TOTAL_COUNT = "totalCount";
	/** Constant for PunchoutConstants */
	public static final String VISIBILITY_ROLE = "visibilityRole";
	/** Constant for PunchoutConstants */
	public static final String REQUEST_DETAILS = "requestDetails";
	/** Constant for PunchoutConstants */
	public static final String BUNDLE_ITEMS = "bundleItems";
	/** Constant for PunchoutConstants */
	public static final String START_POS = "startPos";
	/** Constant for PunchoutConstants */
	public static final String SUPPLY_ITEMS = "supplyItems";
	/** Constant for PunchoutConstants */
	public static final String ACCOUNT_LIST = "accountList";
	/** Constant for PunchoutConstants */
	public static final String SR_NUMBER = "srNumber";
	/** Constant for PunchoutConstants */
	public static final String CART_TYPE = "cartType";
	/** Constant for PunchoutConstants */
	public static final String ACNT_TYPE = "acntType";
	
	public static final String ARIBA_PARAMS = "aribaParamMap";
	
}
