	        ...
	        try {
				...
				if(list != null && !list.isEmpty()) {
					for(int i = 0; i < list.size(); i++){
						...
						if("Account".equals(restrctMdmLevel)){
							count++;
							break;
						}
					}				
...
//			reportSchedule.setEffectiveDate(new Date());
			reportSchedule.setEffectiveDate(TimezoneUtil.getCurrentDateInUTC());
		}
		else{
		...
		String MDMId_LDAP = PortalSessionUtil.getMdmId(request.getPortletSession());
        ...
        contract.setCntry(PortalSessionUtil.getLdapUserData(request.getPortletSession(), LexmarkConstants.COUNTRY));
        contract.setLang(PortalSessionUtil.getLdapUserData(request.getPortletSession(), LexmarkConstants.LANGUAGE));
        contract.setCHL(PortalSessionUtil.getServiceUser(request.getPortletSession()).getChlNodeId());
        contract.setMDMId_LDAP(MDMId_LDAP);
	...
	private static Date populateDate(String strDate, Boolean plus24Hours) throws ParseException {
		if(strDate != null){
			...
			if(plus24Hours){
				...
				LOGGER.debug("Date Returned---  "+serverDate);
			}
			return serverDate;
		}
		return null;
...
//					}
		...
		//Added for LBS Request History
			...
			if(request.getParameter("fromFleetManager")!=null && request.getParameter("fromFleetManager").equalsIgnoreCase("true")){
				...
				if(StringUtils.isNotBlank(cmType)){
				...
				if(StringUtils.isBlank(request.getParameter("searchCriterias")) || ChangeMgmtConstant.CHANGE_REQUESTS.equalsIgnoreCase(request.getParameter("gridType"))){
	...
	/**
	 * @param request 
	 * @return UserFilterSettingContract 
	 * @throws Exception 
	 */
	public static UserFilterSettingContract getFilterSettingContract(PortletRequest request) throws Exception{
		...
		userFilterSetting.setIsPreference(ispref.charAt(0));
		...
		userFilterSetting.setDevice(device);
		userFilterSetting.setServiceRequest(srRequest);
		userFilterSetting.setPrefName(preferenceName);
		userFilterSetting.setDefaultPref(isDefaultpref.charAt(0));
		if(StringUtils.isNotBlank(id)==true){
			userFilterSetting.setRowId(id);
		}		
		contract.setFilterSetting(userFilterSetting);
	...
	public static UserFilterSettingContract getFieldSettingContract(PortletRequest request) throws Exception{
		...
		userFieldViewSetting.setPortalName("SP");
	...
	public static UserFilterSettingContract getFieldSettingContractOPS(PortletRequest request) throws Exception{
		...
		String indexes=request.getParameter("indexes");
		LOGGER.debug(String.format("[indexes length =%s , value= %s \n]",(indexes==null?0:indexes.length()),indexes));
		LOGGER.debug("request.getParameter(\"id\")"+request.getParameter("id"));
	...
	public static DocumentDeleteContract getDocumentOrReportDeleteContract(String reportJobId) {
		DocumentDeleteContract documentDeleteContract = new DocumentDeleteContract();
		documentDeleteContract.setReportJobId(reportJobId);
		return documentDeleteContract;
}
}