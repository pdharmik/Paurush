...
function CommonFilters(){
	...
	this.__displayInfoInPopup=function(prefernceFiltr){
		...
		$('#editPreferencesPopup #default').attr('checked',(prefernceFiltr.getIsDefault()=='Y'?true:false)).trigger('click');
	...
	this.bindCalendarProps=function(){
		...
		$("#"+parentContext+"dateRangeFilterContainer").find("input").mouseup(function(){
			...
			show_cal(this.id, beginDate, endDate);that.__bindCloseCal();
		});
		...
		$('#'+parentContext+'imgLocalizedBeginDateAsset').click(function(){
			show_cal(parentContext+'installDateFrom', null, $('#'+parentContext+'installDateTo').val()==""?null:formatDateToDefault($("#"+parentContext+"installDateTo").val()));that.__bindCloseCal();
		});
		$('#'+parentContext+'imgLocalizedEndDateAsset').click(function(){
			show_cal(parentContext+'installDateTo', $('#'+parentContext+'installDateFrom').val()==""?null:formatDateToDefault($("#"+parentContext+"installDateFrom").val()), todayStr);that.__bindCloseCal();
		});
	};
	...
	this.__bindCloseCal=function (){
		if(popupCal){
			popupCal.user_onchange_handler=function(cal,ob_code){
				cal_on_change(cal,ob_code);
				that.__appednCancelIcon(cal.get_text_field_id());
			};
			
		};
	};
	this.__appednCancelIcon=function (id){
		...
		var fid=id.split(parentContext)[1];
		if(cancelIsprsnt || $('#'+id).val()==allhtmlTextIdsObj[fid]){return;}
		$('#'+id).parent().append("<img src=\"/LexmarkServicesPortal/images/transparent.png\"  class=\"ui_icon_sprite ui-icon-closethick cancelIconCalendar\"  style=\"cursor:pointer;\" />");
		$('.cancelIconCalendar').click(function(){
			$(this).parent().find("input").val(allhtmlTextIdsObj[fid]);
			$(this).remove();
		});
	};
