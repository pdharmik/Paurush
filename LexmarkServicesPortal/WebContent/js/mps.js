...
// Add scroll bar if text size is too long and put the value in JSP
function textScrollExpand(textValue,textId) {
	//alert('inside textScrollExpand textValue='+textValue);
	if(textValue!=null)
	{
		var len = 100;
		if (textValue.length>len) {
			jQuery(textId).addClass('textScroll');
		}
		else {
			jQuery(textId).removeClass('textScroll');			
		}
		jQuery(textId).html(textValue);
	}
	else return;
}

//Add scroll bar if text size is too long
function textScroll(textId) {
	//alert('inside textScroll');
	if(textId!=null)
	{
		var len = 100;
