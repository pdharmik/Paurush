...
|| fn:indexOf(header['user-agent'], 'MSIE 9.0') != -1 || fn:indexOf(header['user-agent'], 'MSIE 10.0') != -1}">
...
${header['user-agent']} showCapture=${showCapture}
  ...
  <div class="error" id="errorDiv" style="display: none;"></div>
   <div class="success" id="successDiv" style="display: none;">
   	<ul id="message_banner_" class="serviceSuccess"><li class="portlet-msg-success"><spring:message code='lbs.alert.accountNotThere'/></li></ul>
   </div>
...
function showPopupMessages(message,callback){
	...
	jQuery(document).ready(function() {
		...
		jQuery('#actionChangeAccount').click(function(){
			callOmnitureAction('<%=LexmarkSPOmnitureConstants.FLEETMANAGER%>','<%=LexmarkSPOmnitureConstants.CHANGEACCOUNT%>');
			showAccountPopup();//method declared in this page.. below...			
		});
		jQuery('#actionCreateRequest').click(function(){
			callOmnitureAction('<%=LexmarkSPOmnitureConstants.FLEETMANAGER%>','<%=LexmarkSPOmnitureConstants.CREATEREQUEST%>');
			popupDialog();//method declared in subTab.jsp			
		});
		
	    jQuery("#applyFilter").click(function(){
	    	callOmnitureAction('<%=LexmarkSPOmnitureConstants.FLEETMANAGER%>','<%=LexmarkSPOmnitureConstants.APPLYFILTER%>');
	...
	function setAccountInformation(accId,accName){
		...
		if(accountDialog!=null){
			...
			//alert('single account show popup');
	...
	$('#clearFilter').click(function(){
		callOmnitureAction('<%=LexmarkSPOmnitureConstants.FLEETMANAGER%>','<%=LexmarkSPOmnitureConstants.CLEARFILTER%>');
