...
/**
 * Service class for Accessories call
 * 
 * @author Ivan Mdzeluri
 * @since 9/15/15
 */

public class GlobalAccessoriesListService {
	private static final Logger logger = LogManager.getLogger(GlobalAccessoriesListService.class);
	...
	public GlobalAccessoriesListService(GlobalCatalogListContract contract) {
		if (contract == null) {
			throw new IllegalArgumentException("contract can not be null");
		}
		this.contract = contract;
		this.agreementId = contract.getAccAgreementId();
		this.productType = contract.getAccProductType();
		this.productModel = contract.getAccProductModel();
		this.partType = contract.getAccPartType();
		this.partNumber = contract.getAccPartNumber();
		this.soldToNumber = contract.getAccSoldToNumber();
		this.paymentType = contract.getAccPaymentType();
		this.contractNumber = contract.getAccContractNumber();
		this.hardwareFlag = contract.isAccHardwareFlag();
		this.effectiveDate = contract.getAccEffectiveDate();
	}
...
//			builder.append(" AND [LXK MPS Material Line] <>'Options'");
		...
		if(isNotBlank(contract.getAccContractNumber())) {
			...
			if(isNotBlank(contract.getAccPaymentType())) {
				childSearchExpression.append(" AND ([LXK MPS Billing Model] = '");
				childSearchExpression.append(paymentType);
				childSearchExpression.append("'");
				
				if(!"Ship and Bill".equalsIgnoreCase(contract.getAccPaymentType())) {
		...
		if(isNotBlank(contract.getAccContractNumber())) {							//child search expression added as per 10936
