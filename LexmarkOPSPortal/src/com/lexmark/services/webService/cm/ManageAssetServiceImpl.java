...
public class ManageAssetServiceImpl implements ManageAssetService {
			...
			throws LGSRuntimeException, LGSBusinessException
	{
		...
		if(serviceReqContract.getServiceRequest().isProject()== true){
			cmServiceReqData.setRequestedService("Install");
			cmServiceReqData.setRequestedServiceAction("Project Based");
			cmServiceReqData.setProjectName(serviceReqContract.getServiceRequest().getProjectName());
			cmServiceReqData.setProjectPhase(serviceReqContract.getServiceRequest().getProjectPhase());
			if(serviceReqContract.getServiceRequest().getSubArea().getValue().equalsIgnoreCase("Install Asset")){
				//HW Install Covered Service
				cmServiceReqData.setCoveredService("HW Install");
				LOGGER.debug("Project Based in IMPL HW Install");
				
			}
			if(serviceReqContract.getServiceRequest().getSubArea().getValue().equalsIgnoreCase("Move Asset")){
				//HW Install Move Covered Service
				cmServiceReqData.setCoveredService("HW Install Move");
				LOGGER.debug("Project Based in IMPL HW Install Move");
			}
			if(serviceReqContract.getServiceRequest().getSubArea().getValue().equalsIgnoreCase("Change Asset")){
				//HW Install Change Covered Service
				cmServiceReqData.setCoveredService("HW Install Change");
				LOGGER.debug("Project Based in IMPL HW Install Change");
			}
			if(serviceReqContract.getServiceRequest().getSubArea().getValue().equalsIgnoreCase("Decommission Asset")){
				//HW Install Decommission Covered Service
				cmServiceReqData.setCoveredService("HW Install Decommission");
				LOGGER.debug("Project Based in IMPL HW Install Decommission");
				
				
			}
			LOGGER.debug("Project Based in IMPL");
		}else{
			cmServiceReqData.setRequestedService(serviceReqContract.getServiceRequest().getArea().getValue());
			cmServiceReqData.setRequestedServiceAction(serviceReqContract.getServiceRequest().getSubArea().getValue());
			LOGGER.debug("Not Project Based in IMPL");
		}
		...
		/*if ("Move Asset".equalsIgnoreCase(serviceReqContract.getServiceRequest().getSubArea().getValue()) || "Map-Internal Portal".equalsIgnoreCase(sourceSystem)) {
