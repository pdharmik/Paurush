...
public class TreeGenerator {
	...
	private StringBuffer generateReportingHierarchyChildXML(List<CHLNode> nodeList){
		...
		if(nodeList != null && !nodeList.isEmpty()){
			...
			childNodesXML.append(" <item child='0' id=''>");
			childNodesXML.append("<itemtext><![CDATA[No Association Available]]></itemtext>");
			childNodesXML.append(" </item>\n");
		}
	...
	public String generateOPSHierarchyXML(List<CHLNode> nodeList, String treeId) {
		StringBuffer nodeTreeXML = new StringBuffer("<?xml version=\"1.0\" ?>\n");
		nodeTreeXML.append("<tree id=\""+treeId+"\">\n");
		nodeTreeXML.append(generateOPSHierarchyChildXML(nodeList));
		nodeTreeXML.append("</tree>");
		//logger.info(nodeTreeXML.toString());
		return nodeTreeXML.toString();
	}

	private StringBuffer generateOPSHierarchyChildXML(List<CHLNode> nodeList){
		String hasChild = NO_CHILD;
		StringBuffer childNodesXML = new StringBuffer("");
		if(nodeList != null && !nodeList.isEmpty()){
			for(int i = 0; i < nodeList.size(); i ++){
				...
				if(node.getHasChild())
					...
					hasChild = NO_CHILD;
				childNodesXML.append(" <item child='"+hasChild+"' id='"+node.getCHLNodeId()+"'>");
				childNodesXML.append(CHL_ITEMTEXT_OPEN + StringEscapeUtils.escapeJavaScript((node.getCHLNodeId())) + JS_FUNCTION_COMMA2 
						+ StringEscapeUtils.escapeJavaScript(node.getChlNodeName()) + JS_FUNCTION_CLOSURE + formatNode(node.getChlNodeName())
						+ ITEMTEXT_CLOSURE);
				childNodesXML.append(" </item>\n");
			}
		}else{
	...
	private DeviceLocationNode getMatchedChildNode(DeviceLocationNode node, String addressValue) {
		...
		item = ITEM_OPEN + formatNode(node.getName().trim())
		/* Commented for MPS 2.1 Wave 4*/
		//+ formatNode(node.getParentname())
		//Ends Commented for MPS 2.1 Wave 4
		+ ITEM_OPEN_END +
