...
public class XmlOutputGenerator {
	...
	public String convertAssetToXML(ResourceRequest request,List<Asset> assets, int totalCount, int posStart, String contextPath, String userSiebelContactId, String type) {
		...
		for (Asset asset : assets) {
			...
			if(!"false".equalsIgnoreCase(showViewMore) && !hidePgCnts){
		...
		String quantity = "";
			...
			for(OrderPart orderPart : catalogPartList)
			{
				...
				/*if(implicitFlag){
					...
					xml.append("<div class=\"pModel w100\"><img src=\"/LexmarkOPSPortal/images/printer_implicit_part.png\" width=\"100\" height=\"100\"/></div>");
				}*/
				...
				if(partImage!=IMAGE_NOT_FOUND){
					...
					xml.append("<div class=\"pModel w100\"><img src=\"/LexmarkOPSPortal/images/part_na_color.png\" width=\"100\" height=\"100\"/></div>");
				}				
				xml.append("]]></cell>\n");
		...
		boolean nullFlag=false;
			...
			for(OrderPart accessory : accessoriesList)
			{
				...
				if(partImage!=IMAGE_NOT_FOUND){
					...
					partImage = "";
					xml.append("<img src=\'/LexmarkOPSPortal/images/part_na_color.png\' width=\'100\' height=\'100\'/>");
				}
	...
	public String buildXMLForChangeRequestType(List<ServiceRequest> serviceRequestList, int totalCount, int startRecordNumber, float timezoneOffset){
		...
		//	logger.debug("areaLOV---->["+areaLOV.getValue()+"]");
			...
			xml.append("  <cell><![CDATA["+replaceNullWithBlankString(serviceRequest.getRequestStatus() )+"]]></cell>\n");				
			xml.append("  <cell><![CDATA["+ replaceNullWithBlankString(serviceRequest.getSubStatus())+"]]></cell>\n");
			xml.append("  <cell><![CDATA["+replaceNullWithBlankString(serviceRequest.getSeverity() )+"]]></cell>\n");
			xml.append("  <cell><![CDATA["+ replaceNullWithBlankString(serviceRequest.getProjectName() )+"]]></cell>\n");
			xml.append("  <cell><![CDATA["+ replaceNullWithBlankString(serviceRequest.getProjectPhase() )+"]]></cell>\n");			

			
			
			xml.append("  <cell><![CDATA["+ replaceNullWithBlankString(serviceRequest.getAgreementName() )+"]]></cell>\n");
			xml.append("  <cell><![CDATA["+ replaceNullWithBlankString(serviceRequest.getAgreementNumber() )+"]]></cell>\n");			
			xml = xml.append(" </row>\n");
	...
	private String getReportActionsString(String categoryName, String reportDefinitionName,ReportListRow reportRow, String contextPath, boolean canDeleteReport){
		...
		String mailLinkPartB = "')\">" + IMG_GRAYED_OPEN + contextPath + IMAGE_PATH + EMAIL_FILE_NAME + "\"/></a>";
		String delLinkPartAPattern = "<a href='#' title=''{0}''onclick=\"deleteReport(";
		String delLinkPartB = ")\">" + IMG_GRAYED_OPEN + contextPath + IMAGE_PATH + DELETE_FILE_NAME + "\"/></a>";
		...
		for (int i = 0;i< list.size() ; i++) {
			...
			if((!"".equals(list.get(i).getEmailSubject()) && list.get(i).getEmailSubject()!=null)
				...
				xml.append("<cell><![CDATA[<a href=\"javascript:void(0)\" onClick=\"editEmailNotificationLocale('" +list.get(i).getEmailNotificationLocaleId()+"')\">"+ editMessage + "</a>&nbsp;"+IMG_OPEN + imageDeleteSrc +
		                "\" onClick=\"deleteEmailNotificationLocale('"+list.get(i).getEmailNotificationLocaleId()+"')\"/>]]></cell>\n");
			}else{
	 ...
	 * @return String 
	 */
	public String convertReportsToXML(ReportInstanceListResult result, int totalCount, int posStart){
		...
		for(int i = 0; i < reportList.size(); i ++){
			...
			String runTime = formatDateTime(reportList.get(i).getRunTime());
			xml = xml.append("<cell><![CDATA[" + runTime + "]]></cell>\n");
			xml = xml.append("<cell><![CDATA[" + userEmail + "]]></cell>\n");
			xml = xml.append("<cell><![CDATA[" + reportList.get(i).getDefinition().getName() + "]]></cell>\n");
			xml = xml.append("<cell><![CDATA[" + localizeReportStatus(reportList.get(i).getStatusCode(), locale) + "]]></cell>\n");
			...
			xml = xml.append("<cell><![CDATA[<a href=\"javascript:void(0)\" onClick=\"hideSelect();goDetail('"+reportList.get(i).getRunTime()+"','"
				 +reportList.get(i).getDefinition().getName()+"','"+reportList.get(i).getJobRunLogId().toString()+"')\"; title=\""+ getDetailToolTip(reportList.get(i))+"\">Details</a>]]></cell>\n");
			if(reportList.get(i).getStatusCode()!=null && reportList.get(i).getStatusCode().equals(RunLogStatusEnum.PUBLISHOK.getStatusCode())){
				xml = xml.append(" <cell><![CDATA[ <button class=\"button\" onclick=\"reRun('" +
						reportList.get(i).getDefinition().getId().toString()+"');\">" +
						PropertiesMessageUtil.getPropertyMessage(LexmarkSPConstants.MESSAGE_BUNDLE_NAME,"reportAdmin.label.rerun", locale) +
						"</button>]]></cell>\n");
			} else {
				xml.append("<cell></cell>\n");
			}
...
//			xml.append(IMG_OPEN + contextPath + IMAGE_PATH + DELETE_FILE_NAME + "\" onClick=\"deleteRowInGrid()\"/>]]></cell>\n");
			xml.append(IMG_OPEN + contextPath + IMAGE_PATH + DELETE_FILE_NAME + "\" />]]></cell>\n");
			xml.append(" </row>\n");
			i ++;
		}
		xml.append("</rows>\n");
	...
	public String sapURLListViewXml(PortletSession session, List<SapPortlet> billerDirectList, String contextPath){
		...
		if(billerDirectList.get(i).getSapPortletURLs()!=null){
			for(int j=0;j<billerDirectList.get(i).getSapPortletURLs().size();j++){
				...
				xml.append("<td><img id=\"billerDirectURLList["+i+"].billerDirectList["+j+"]Img\" src=\'"+imageDeleteSrc+"' onClick=\"deleteSubTabRow(this,"+i+","+j+")\" style=\"cursor:pointer;\"></td>");
				xml.append("</tr>");
			}
		}
		...
		xml.append("<cell><![CDATA[<a href=\"javascript:deleteGridRow(\'"+i+"');\"><img style=\"float:none;\" src=\'"+imageDeleteSrc+"'  width=\"15\" height=\"15\"/>]]></cell>\n");//Delete
		xml.append(" </row>\n");
		}
		xml.append("</rows>\n");
