...
<portlet:renderURL var="forwardToChangeAddressConfirmation">
...
<portlet:renderURL var="setOldAddressSession">
  ...
  <div id="content-wrapper">
    ...
    <div class="main-wrap">
      <div class="content">
          ...
          <!-- MAIN CONTENT BEGIN -->
          <h3 class="pageTitle"><spring:message code="requestInfo.cm.manageAddress.heading.addressChange"/></h3>
		  ...
		  <!-- Added for CI 14-02-03 ENDS-->      
      	 ...
      	 <hr class="separator" />
		  <p class="info"><spring:message code="requestInfo.cm.manageAddress.heading.infoToUpdateAddress"/>
		 
		  <span style="font-weight:normal!important;font-style: italic;font-size: 12px;" ><spring:message code="requestInfo.label.fieldsMarked"/> <span class="req">*</span><spring:message code="requestInfo.label.areRequired"/></span>
		  <span style="font-weight:normal!important;font-style: italic;font-size: 12px;"><spring:message code="requestInfo.label.fieldsMarked"/> <span class="req">**</span><spring:message code="requestInfo.label.provideAtleastOne"/></span>
		</p>
		  ...
		  <div class="portletBlock infoBox rounded shadow split">
			<div class="columnsTwo">
				<div class="columnInner">
					...
					<h4><a href="<portlet:renderURL/>" title="Select Different Address" id="lightboxAddress"><spring:message code="requestInfo.link.selectADifferentAddress"/></a>
					...
					<ul class="form">
					  ...
					  <li>
                        <label><spring:message code="requestInfo.addressInfo.label.officeNumber"/></label>
					  ...
					  <li>
                        ...
                        <span>${addressDetailsOld['zipPostal']}</span>
                      </li>
					</ul>
				</div>
			</div>
	
			<div class="columnsTwo columnRt">
				<div class="columnInner">
					<h4> <spring:message code="requestInfo.cm.manageAddress.heading.updateReq"/> </h4>
					
                    <ul id="inputValuesArea" class="form">
					  <li>
                        <label for="addName"><spring:message code="requestInfo.addressInfo.label.addressName"/> </label>
                        <span><input type="text" id="addressName"  maxlength="100"/>
						<!--  Help Icon Title changed for 14.8 and 14.9 Translations -->
						<img class="helpIcon ui_icon_sprite info-icon"  src="<html:imagesPath/>transparent.png" title="<spring:message code="requestInfo.tooltip.changeAddress.addressName"/>" ></span> 
                      </li>
					  <li>
                        <label for="storeName"><spring:message code="requestInfo.addressInfo.label.storeFrontName"/></label>
                        <span><input type="text" id="storeFrontName"  maxlength="50"/></span>
					  ...
					  <li>
                        <label for="city"><spring:message code="requestInfo.addressInfo.label.city"/>
                        <span	class="req">*</span>
                        </label>
                        <span><input type="text" id="city"  maxlength="50"/></span>
                      </li>
					  ...
					  <li>
                        ...
                        <span><select id="country" style="width: 190px;" onchange="loadStateList();"></select></span>
                      </li>
					  <li>
                        <label for="state"><spring:message code="requestInfo.addressInfo.label.region"/>
                        <span class="req">**</span>
                        </label>
                        <span><select  id="state" style="width: 190px;" disabled="disabled"></select>
                        <img src="<html:imagesPath/>loading-icon.gif" id="loadingImage" style="display: none;">
                        </span>
                        <div id="stateorzipmesg" class="note" ><span style="margin-top:-8px;">(<spring:message code="requestInfo.addressInfo.label.regionDetails"/>)</span></div>
                      </li>
                      <c:choose>
                      	<c:when test="${addressDetailsOld['houseNumber'] != ''}">
                      	<li>
                        <label for="officeNumber"><spring:message code="requestInfo.addressInfo.label.officeNumber"/></label>
                        <span><input type="text" id="officeNo"  maxlength="50"/></span>
                      </li>
                      	</c:when>
                      	<c:otherwise>
                      ...
                      <li>
                        <label><spring:message code="requestInfo.changemgmt.address.county"/></label>
                        <span><input type="text" id="county"  maxlength="50"/></span>
                      </li>
                      <li>
                        <label><spring:message code="requestInfo.changemgmt.address.district"/></label>
                        <span><input type="text" id="district"  maxlength="50"/></span>
                      </li>
                      
                      
					  <li>
                        <label for="zipCode"><spring:message code="requestInfo.addressInfo.label.postalCode"/>
                        <span class="req">**</span>
                        </label>
...
var portlet_name="<%=LexmarkSPOmnitureConstants.CHANGEADDRESS%>";
var action_primary="<%=LexmarkSPOmnitureConstants.CHANGEADDRESSSELECTADIFFERENTCONTACT%>";
var action_secondary="<%=LexmarkSPOmnitureConstants. CHANGEADDRESSADDADDITIONALCONTACT%>";
var pageType = "changeAddress"; //Added for page type for CI 14-02-03
var callCleansing=true; 
var cleaseAddressFields={
		inputIds:["addressName","storeFrontName","addrLine1","addrLine2","officeNo","city","country","state","zipCode","county","district"],
		keys:["addressName","storeFrontName","addressLine1","addressLine2","officeNumber","city","country","state","postalCode","county","district","province","countryISOCode","region","stateCode","stateFullName","latitude","longitude","savedErrorMessage","isAddressCleansed"]
};
...
function setValues(){
	...
	//This check is done if user doesnot enter any values in text fields 
	//and click on continue...
	//&&jQuery('#addrLine3').val()==""
	if(jQuery('#addressName').val()==""
		&&jQuery('#storeFrontName').val()==""
		&&jQuery('#addrLine1').val()==""
		&&jQuery('#addrLine2').val()==""
		&&jQuery('#city').val()==""
		&&jQuery('#zipCode').val()==""
		&&jQuery('#country option:selected').text()==""
		&&jQuery('#state option:selected').text()==""
		&&jQuery('#county').val()==""
		&&jQuery('#district').val()==""){
	...
	validationFlag=validateAddress();
	...
	if(validationFlag==false)
	{
		...
		copyCleansedValuesToInput();
		var changeAddressConfirmUrl="${forwardToChangeAddressConfirmation}&isChangeAddress=true&timeZoneOffset=" + timezoneOffset;
		jQuery("#addressForm").attr("action", changeAddressConfirmUrl);
		jQuery('#addressForm').submit();
		}
	}
	
}
//Ajax call for loading country list
...
//ajax call for loading state list according to country
...
function goForAddressCleansing(errorDivId){
	...
	jQuery.ajax({
			...
			data: {
					...
					<%--
					<%=ADDRESSCLEANSEFIELDS_INPUT[0]%>: jQuery('#storeFrontName').val()==""?"${addressDetailsOld['storeFrontName']}":jQuery('#storeFrontName').val(),
					<%=ADDRESSCLEANSEFIELDS_INPUT[1]%>: jQuery('#addrLine1').val()==""?"${addressDetailsOld['addressLine1']}":jQuery('#addrLine1').val(),
					<%=ADDRESSCLEANSEFIELDS_INPUT[2]%>: jQuery('#addrLine2').val()==""?"${addressDetailsOld['addressLine2']}":jQuery('#addrLine2').val(),
					<%=ADDRESSCLEANSEFIELDS_INPUT[3]%>: jQuery('#city').val()==""?"${addressDetailsOld['city']}":jQuery('#city').val(),
					<%=ADDRESSCLEANSEFIELDS_INPUT[4]%>:	jQuery('#country option:selected').val()==""?"${addressDetailsOld['country']}":jQuery('#country option:selected').val(),
					<%=ADDRESSCLEANSEFIELDS_INPUT[5]%>:	jQuery('#state').is(':visible')==false?"${addressDetailsOld['stateGrid']}":jQuery('#state option:selected').val()==""?"${addressDetailsOld['stateGrid']}":jQuery('#state option:selected').val(),
					<%=ADDRESSCLEANSEFIELDS_INPUT[6]%>: jQuery('#zipCode').val()==""?"${addressDetailsOld['zipPostal']}":jQuery('#zipCode').val(),
					<%=ADDRESSCLEANSEFIELDS_INPUT[7]%>: jQuery('#officeNo').val()--%>


					<%=ADDRESSCLEANSEFIELDS_INPUT[0]%>: jQuery('#storeFrontName').val(),
	...
	if(jQuery('#checkCleanse').attr('checked')){
		for(i=0;i<cleaseAddressFields.keys.length;i++){
				...
				if(cleaseAddressFields.keys[i]=="isAddressCleansed"){
					...
					if(cleaseAddressFields.keys[i]=="addressName"){
						jQuery("[name='"+inputHTMLId+"']").val(jQuery('#'+cleaseAddressFields.inputIds[0]).val());
...
function validateAddress(){
	...
	*/
	...
	if((jQuery('#zipCode').val()=="")&&(jQuery('#state option:selected').text()=="")){
