...
<portlet:actionURL var="changeAssetConfirmationUrl">

<portlet:param name="action" value="changeMultiAssetConfirmation"></portlet:param>
</portlet:actionURL>
		...
		<div class="content">
		 ...
		 <ul class="form installCheck">
							<li>
								 ...
								 <spring:message code="requestInfo.tooltip.`RequireLexmarkToCollectThisAsset"/> --%>  
								 <spring:message code="requestInfo.cm.manageAsset.label.doYouRequireLexmarkToPhysicallyMoveThisDeviceMultiSelect"/>
								 <span class="req">*</span> 
								 </label> 
								 <span class="radio radio_confirm">
								 <form:radiobutton name="moveAsset" path="installAssetFlag" value="yes" id="moveAssetYes" onclick="clearValidatnErr()"/>
								 <label>
								 <spring:message code="requestInfo.option.yes"/> </label> 
								 <form:radiobutton name="moveAsset" path="installAssetFlag" value="no" id="moveAssetNo" onclick="clearValidatnErr()"/>
								 <label>
...
jQuery(document).ready(function() {
	...
	var changeAssetConfirmUrl="${changeAssetConfirmationUrl}"+"&timeZoneOffset=" + timezoneOffset+"&pageFlow="+flowPage+"&fleetManagementFlag="+fleetManagegmtFlag+"&multiAssetList="+multiAssetList;
	...
	jQuery("#btnContinue").click(function(){
		callOmnitureAction('<%=LexmarkSPOmnitureConstants.CHANGEMULTIASSET%>','<%=LexmarkSPOmnitureConstants.REVIEWCHANGEMULTIASSETSUBMIT%>');
		...
		jQuery("#errorDiv").html('');
		 ...
		 var elementIdsToValidate=["addtnlDescription","effDtOfChange"];
			...
			/*if(lbsaddressflagins=="true"){
				alert("lbsaddressflagins");
			
				//alert("floor"+jQuery('#flr1').val());
			...
			//alert("lbsaddressflagmov"+lbsaddressflagmov);
				...
				for(var ind=0; ind <assetArr.length; ind++)
				{
					if(assetLbsAddressFlag=="true" && addressLevelOfDetails.match("floor level|grid level|mix - see floor")){
						...
						if(jQuery('#'+assetArr[ind]+'_physicalLocation1h').val() == ''|| jQuery('#'+assetArr[ind]+'_physicalLocation1h').val() ==null ){
							validationflag=true;
							jQuery("#errorDiv").append("<li class=\"portlet-msg-error\"><strong><spring:message code='lbs.multiselect.asset.building'/> "+$("#"+assetArr[ind]+"_serialNumber").val()+"</strong></li>");
						}
						if(jQuery('#'+assetArr[ind]+'_physicalLocation2h').val() == ''|| jQuery('#'+assetArr[ind]+'_physicalLocation2h').val() ==null){
							validationflag=true;
							jQuery("#errorDiv").append("<li class=\"portlet-msg-error\"><strong><spring:message code='lbs.multiselect.asset.floor'/> "+$("#"+assetArr[ind]+"_serialNumber").val()+"</strong></li>");
						}
					}
			...
			var fleetMgmtFlag = "${fleetManagementFlag}";
				...
				if((jQuery('#installAssetYes').is(':checked')))
					...
					if(fleetMgmtFlag!= null && fleetMgmtFlag== 'true'){
						...
						if(pageFlow == "fleetMgmtMove" && (lbsaddressflagmov=="true" && levelOfDetails.match("floor level|grid level|mix - see floor"))){
				...
				if(!(jQuery('#moveAssetYes').is(':checked')) && !(jQuery('#moveAssetNo').is(':checked')))
				{	
					validationflag=true;
...
function showMapPopup(id){
	mapPopupObj.showPopup({
		"id":id,
		"buildingId":"1-NTD3WVP",
		"floorId":"",
		"successFunc":successMap,
		"encryptionURL":"${encryptJSONVar}",
		"lbsPostURL":"${formPost}",
		"locale":"<%=request.getLocale()%>",
		"mdmId":"071695183",//"${mdmId}";
		"mdmLevel":"Global",//"${mdmLevel}";
		"emailAddress":"${emailAddress}"
	});
	return false;
}
function successMap(obj){
	console.log('in success' + JSON.stringify(obj));
}
