...
<div id="divFilter" onClick="showOrHideFilter()"><span><spring:message code='lbs.label.filters'/></span><img id="arrow" src="<html:imagesPath/>arrow-down-grayed.png"/></div>
			...
			<div id="filterLocation">
					...
					<div id="filterCountryStateCity">
						...
						<a id="addRessLink" onClick="openAddressPopup()" style="cursor: pointer;"><spring:message code='lbs.label.selectaddress'/></a>
						</div>
					</div>
				...
				<div id="customerHierarchy">
					<div class="filterHeader">
						...
						<a id="customerHierarchyDir" onclick="popUpChlTree()" style="cursor: pointer;"><spring:message code='lbs.label.customerhierarchy'/></a><br/>
						<span id="chlNodeValueLabel"></span>						
					</div>
				
				<div id="filterDevice">
				<div class="filterHeader" id="filterDeviceHeader">
				...
				<div id="DeviceProperties">
					<ul>
						...
						<input id="productModel" type="text" name="productModel" value="<spring:message code='lbs.fleetmgmt.ops.model'/>" />
						</li>				
						<li >
						...
						<li>
						<input id="modelType" type="text" name="modelType" value="<spring:message code='lbs.label.modeltype_family'/>" />
						</li>	
						...
						<%-- Below fields will be viewed by Employees only!! --%>						
						<li>
							<input type="text" id="LXKAssetTag" value="<spring:message code='lbs.label.LXKAssetTag'/>"/>
						</li>
						<li>
							<div id="assetLifeCycle"></div>
							 
						</li>
						<li>
						<div id="devicePhase"></div>
						...
						<li>
							<input type="text" id="mdmID" name="mdmId" value="<spring:message code='lbs.label.MDMID'/>" />
						</li>
						<%-- This fields will be viewed by Customer!! --%>
						<li>
						...
						<li style="clear:both;width:100%">						
							<ul id="assetInstallDateDiv" style="display: inline;"><%-- Don't remove this DIV  --%>					
								...
								<img id="imgLocalizedBeginDateAsset" src="<html:imagesPath/>transparent.png" class="ui_icon_sprite calendar-icon"  style="cursor:pointer;" />
								</li>
								...
								<img id="imgLocalizedEndDateAsset" src="<html:imagesPath/>transparent.png" class="ui_icon_sprite calendar-icon"  style="cursor:pointer;" />
								</li>
							</ul>
						<li>
						</ul>
				...
				<div id="filterPlacement">
						<div class="filterHeader">
							<h3>Future Placements</h3>
						</div>
						<div id="placementProperties">
						<ul>
						<li>							
							<div id="placementDiv">
							<select id="placementSelect" onchange="placementObj.onChangeHandler()"></select>
				...
				<div id="RequestDetails">
					<ul>
						...
						<li>
							<div id="srOPSStatus"></div>
						</li>
						<li>
							<div id="srSubStatus"></div>
						</li>
						<li>
							<div id="srSource"></div>
						</li>
						<li style="clear:both;width:100%">
							<ul id="dateRangeFilterContainer"><%-- Don't remove this DIV  --%>					
								...
								<img id="imgLocalizedBeginDate" src="<html:imagesPath/>transparent.png" class="ui_icon_sprite calendar-icon"  style="cursor:pointer;" />
								</li>
								...
								<img id="imgLocalizedEndDate" src="<html:imagesPath/>transparent.png" class="ui_icon_sprite calendar-icon"  style="cursor:pointer;" />
								</li>
							</ul>
						</li>
					</ul>		
...
function allhtmlTextIds(){
	...
	this.productModel="<spring:message code='lbs.fleetmgmt.ops.model'/>";
	<%--17366 Changes  
	...
	this.LXKAssetTag="<spring:message code='lbs.label.LXKAssetTag'/>"; 
	//this.assetLifeCycle="AssetLifeCycle"; 
	//this.devicePhase="DevicePhase";
	//this.hardwareStatus="HardwareStatus";
	this.mdmID="<spring:message code='lbs.label.MDMID'/>";
	
	
	
};

 var deviceFilterFields={
		"serialNumber":"<spring:message code='lbs.label.serialno'/>",
		"ipAddress":"<spring:message code='lbs.label.ipaddress'/>",
		"productModel":"<spring:message code='lbs.fleetmgmt.ops.model'/>",
		"assetProduct":"<spring:message code='lbs.label.product'/>",
		"installDateFrom":"<spring:message code='lbs.fleetmgmt.installDate.From'/>",
		"installDateTo":"<spring:message code='lbs.fleetmgmt.installDate.To'/>",
		"deviceHostName":"<spring:message code='lbs.label.devicehostname'/>"
}
...
function deviceIdsHTML(){
	...
	//this.singleMFP="<spring:message code='lbs.label.single_mfp'/>";
		<%-- Below fields are to be seen by only employee--%>
		
		this.LXKAssetTag="<spring:message code='lbs.label.LXKAssetTag'/>";
		this.assetLifeCycle="<spring:message code='lbs.label.assetlifecycle'/>";
		this.devicePhase="<spring:message code='lbs.label.devicephase'/>";
		this.hardwareStatus="<spring:message code='lbs.label.hardwarestatus'/>";
		this.mdmID="<spring:message code='lbs.label.MDMID'/>";
...
var srOPSStatusArray = [];
srOPSStatusArray.push({"value":"","displayValue":"<spring:message code='lbs.label.srOPSStatus'/>"});
<c:forEach items="${srOPSStatus}" var="requestStatus" varStatus = "status" >
srOPSStatusArray.push({"value":"${requestStatus.key}","displayValue":"${requestStatus.value}"});
</c:forEach>

var srSubStatusArray = [];
srSubStatusArray.push({"value":"","displayValue":"<spring:message code='lbs.label.srSubStatus'/>"});
<c:forEach items="${srSubStatus}" var="requestStatus" varStatus = "status" >
srSubStatusArray.push({"value":"${requestStatus.key}","displayValue":"${requestStatus.value}"});
</c:forEach>

var srSourceArray = [];
srSourceArray.push({"value":"","displayValue":"<spring:message code='lbs.label.srSource'/>"});
<c:forEach items="${srSource}" var="requestStatus" varStatus = "status" >
srSourceArray.push({"value":"${requestStatus.key}","displayValue":"${requestStatus.value}"});
</c:forEach>

var assetLifeCycleArray=[];
assetLifeCycleArray.push({"value":"","displayValue":"<spring:message code='lbs.label.assetlifecycle'/>"});
<c:forEach items="${assetLifeCycle}" var="requestStatus" varStatus = "status" >
assetLifeCycleArray.push({"value":"${requestStatus.key}","displayValue":"${requestStatus.value}"});
</c:forEach>
var devicePhaseArray=[];
devicePhaseArray.push({"value":"","displayValue":"<spring:message code='lbs.label.devicephase'/>"});
<c:forEach items="${devicePhase}" var="requestStatus" varStatus = "status" >
devicePhaseArray.push({"value":"${requestStatus.key}","displayValue":"${requestStatus.value}"});
</c:forEach>
var hardwareStatusArray=[];
hardwareStatusArray.push({"value":"","displayValue":"<spring:message code='lbs.label.hardwarestatus'/>"});
<c:forEach items="${hwStatus}" var="requestStatus" varStatus = "status" >
hardwareStatusArray.push({"value":"${requestStatus.key}","displayValue":"${requestStatus.value}"});
</c:forEach>
<%-- Added for LBS 1.5 --%>
var addressLOD = {};
...
var widthAuto="auto";
	...
	var srOPSStatusObj=new MultiSelect({
		"elementId":'srOPSStatus',
		"elementWidth":width,
		"dataList":srOPSStatusArray
	});
	
	var srSubStatusObj=new MultiSelect({
		"elementId":'srSubStatus',
		"elementWidth":width,
		"dataList":srSubStatusArray
	});
	
	var srSourceObj=new MultiSelect({
		"elementId":'srSource',
		"elementWidth":width,
		"dataList":srSourceArray
	});
	
	var lifeCycleObj=new MultiSelect({
		"elementId":'assetLifeCycle',
		"elementWidth":width,
		"dataList":assetLifeCycleArray
	});
	
	var devicePhaseObj=new MultiSelect({
		"elementId":'devicePhase',
		"elementWidth":width,
		"dataList":devicePhaseArray
	});
	
	var hardwareStatusObj=new MultiSelect({
		"elementId":'hardwareStatus',
		"elementWidth":width,
		"dataList":hardwareStatusArray
	});
	
var placementArray = [];
	//placementArray.push({"key":"","value":"--Select--"});
	placementArray.push({"key":"on","value":"On"});
	placementArray.push({"key":"off","value":"Off"});
	placementArray.push({"key":"only","value":"Placements Only"});
...
jQuery(document).ready(function(){
	...
	// added  for placements
	var select = $("#placementSelect");
	select.empty();
	$.each(placementArray , function(list) {
	    var content='<option  value="' + placementArray[list].key + '">' + placementArray[list].value + '</option>';
...
function loadPopupCustomize(x)
{
	...
	if($(x).attr('id')=="customizeDevice")
	{
		...
		var finalHtml="<div class=\"popupHeader popupSubHeader\"><span><b>Customer</b></span></div><table><tr>";
		var countt=0;
	...
	if($(x).attr('id')=="customizeRequests")
	{
		...
		if($('#srOPSStatus').css('display')!="none"){strSrOPSStatus=checked;}
		if($('#srSubStatus').css('display')!="none"){strSrSubStatus=checked;}
		if($('#srSource').css('display')!="none"){strSrSource=checked;}
		...
		$("#loadCustomizeSelectors").html("<input class=\"customize\" type=\"checkbox\" value=\"areaDrop\" "+strArea+"onclick=\"OpenCustomizePopup(this)\"><spring:message code='lbs.label.area'/><br>"+
											...
											"<input class=\"customize\" type=\"checkbox\" value=\"srOPSStatus\" "+strSrOPSStatus+" onclick=\"OpenCustomizePopup(this)\"><spring:message code='lbs.label.srOPSStatus'/><br>"+
											"<input class=\"customize\" type=\"checkbox\" value=\"srSubStatus\" "+strSrSubStatus+" onclick=\"OpenCustomizePopup(this)\"><spring:message code='lbs.label.srSubStatus'/><br>"+
											"<input class=\"customize\" type=\"checkbox\" value=\"srSource\" "+strSrSource+" onclick=\"OpenCustomizePopup(this)\"><spring:message code='lbs.label.srSource'/><br>");
		widthpopupCustomize=$('#popupCustomize').width();
...
function bindToBeginDateImgClick(){
	show_cal('installDateFrom', null, $('#installDateTo').val()==""?null:formatDateToDefault($("#installDateTo").val()));
	bindCloseCal();
};
function bindToEndDateImgClick(){
	show_cal('installDateTo', $('#installDateFrom').val()==""?null:formatDateToDefault($("#installDateFrom").val()), todayStr);
	bindCloseCal();
};

$('#imgLocalizedBeginDateAsset,#installDateFrom').click(bindToBeginDateImgClick);
$('#imgLocalizedEndDateAsset,#installDateTo').click(bindToEndDateImgClick);
...
var cannedMsgs=["<spring:message code='lbs.filter.canned0'/>","<spring:message code='lbs.filter.canned1'/>","<spring:message code='lbs.filter.canned2'/>","<spring:message code='lbs.filter.canned3'/>","<spring:message code='lbs.filter.canned4'/>","<spring:message code='lbs.filter.canned5'/>"];
var cannedObj=[];
var cannedString="";
...
function loadCanned(id){
	...
	showPopupMessages("Are you sure you want to load this filter?\
			All your current filters will be lost and new filters will be loaded",function(){
				resetFilters(false);
				if($('#left-nav-Div').css('display')!='none'){
	    			hideNav();<%-- Hide left navigation --%>	
	    		}	
