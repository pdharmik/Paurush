            ...
            <div class="columnInner">
			  ...
			  <div class="${className} floatL">
				...
				<c:if test='${isHwMove}'>
						...
						<div class="${className} separatorV floatL min-height-150px" >
			   				 <table width="100%" border="0" cellspacing="0" cellpadding="0" class="dataGrid">
			                ...
			                <c:if test="${not empty moveToAddressDisplay.physicalLocation1}">
			                <div><spring:message code="claimCreate.addressInfo.label.building"/>${moveToAddressDisplay.physicalLocation1}</div>
			                
			                </c:if>
			                <c:if test="${not empty moveToAddressDisplay.physicalLocation2}">
							<div><spring:message code="claimCreate.addressInfo.label.floor"/>${moveToAddressDisplay.physicalLocation2}</div>
							
							</c:if>
							<c:if test="${not empty moveToAddressDisplay.physicalLocation2}">
							<div><spring:message code="claimCreate.addressInfo.label.office"/>${moveToAddressDisplay.physicalLocation3}</div>
			  ...
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="dataGrid ">
			  	<tr>
			  		<td width="200" class="label leftBlock"><spring:message code="requestInfo.hardwareDebreief.servInfo.helpdeskRefNo"/></td>
			  		<%-- Changes on field mapping on 24th jan --%>
			  		 <td>${hardwareDebriefForm.activity.serviceRequest.customerReferenceNumber}</td>
                ...
                <tr>
                  ...
                  <td>
                  <input id="expectedEndDate"  type="text" disabled="disabled" name="userEnteredActivity.serviceRequest.resolveWithin" value="<util:dateFormat value="${hardwareDebriefForm.userEnteredActivity.serviceRequest.resolveWithin}" showTime="true"  timezoneOffset="${tzOffset}"/>"/>
                  <img class="ui-icon calendar-icon" src="<html:imagesPath/>transparent.png" id="resolveWithin_cal" title="<spring:message code="requestInfo.hardwareDebreief.common.closeOut.closeOutInstallActivity.expectedEndDate"/>" alt="Remove" onclick="show_cal('expectedEndDate', null, null,true);">            
                  </td>
