...
<c:set var="hwDeInstall" value="${isHwDeInstall}" scope="request"/>
<%-- 
...
<portlet:actionURL var="submitDEInstallDebriefRequestURL">
	<portlet:param name="action" value="submitDeInstallRequest"/>
</portlet:actionURL>
...
<div id="wrapper">
  <div id="content-wrapper">
    <div class="journal-content-article">
      <h1><spring:message code="requestInfo.hardwareDebreief.debriefDeInstall.DeInstallReq.heading"/></h1>
    </div>
    <div class="main-wrap">
    ...
    <div style="display: none;" id="errorDiv" class="error"></div>
      ...
      <div class="grid-controls"> 
        ...
        <!-- MAIN CONTENT GOES HERE -->
         ...
         <form:form id="hardwareDebriefForm" method="post" commandName="hardwareDebriefForm" modelAttribute="hardwareDebriefForm" action="${submitDEInstallDebriefRequestURL}">
         <form:input type="hidden" path="<%=TIMEZONE_OFFSET%>"  id="<%=TIMEZONE_OFFSET%>" value="${tzOffset}"/>
       ...
       <jsp:include page="/WEB-INF/jsp/hardwareDebrief/HWInstallDeInstallMove/commonHwView/commonSuppliesInfoForDeviceView.jsp"/>
       --%> 
		...
		<c:if test="${hardwareDebriefForm.activity.serviceRequest.serviceRequestStatus != 'completed'}">
          <a class="button button21"  style="position:static!important;" onclick="saveDeinstall()"><spring:message code="button.save_as_draft" /></a>
          
          <button style="position:static!important;" class="button button21" type="button" onclick="confirmInstallCloseOut()"><spring:message code="button.closeout" /></button>
          </c:if>
			...
			function confirmInstallCloseOut(){
			    ...
			    jConfirm(confirmationMessages.ConfirmMsg2, confirmationMessages.ConfirmTitle, function(r)
			    	...
			    	if(r==true){
						...
						if(!checkMandatoryFields(mandatoryFieldObj) && !doCustomerProfileValidation(customerProfileFields,inputNamesMapToSiebel,'deInstall')){
