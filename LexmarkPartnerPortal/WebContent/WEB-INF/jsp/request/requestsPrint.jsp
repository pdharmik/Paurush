...
<table width="100%">
	<tr id="topPrint">
		<td align="left">
		&nbsp;&nbsp;<a href="javascript:void(0)" onClick="doPrint();" id="topPrintLink"><img src="<html:imagesPath/>transparent.png" class="ui-icon print-icon cursor-pointer" height="23px" width="23px" title="<spring:message code='button.clickToPrint'/>" /><span class="cursor-pointer"><spring:message code='button.clickToPrint'/></span></a>
		</td>
	</tr>
	<tr>
		<td align="center" >
...
<script type="text/javascript">
	...
	removeClass(document.getElementById("content-wrapper"), "columns-max")
	...
	var columnsNum = parentgrid.getColumnsNum()-1;
	if(columnsNum%2 == 0){
		var halfColumnsNum = columnsNum/2;
	}else{
		halfColumnsNum = parseInt(columnsNum/2)+1;
	}

	var gridContent = "<br><table border='1px'  cellspacing='0px' width='auto' class='print'><tr>";
	gridContent +=  "<th align='left'>&nbsp;</th>";
	for(m=1;m<halfColumnsNum+1;m++){
	...
	for(n=start;n<(start+printRecordNumber);n++){
		...
		gridContent += "<tr>";
		for(i = 0;i<halfColumnsNum+1;i++){
		
			gridContent += "<td align='left'>";
			gridContent += i==0?n+1:parentgrid.cellByIndex(n, i).cell.innerHTML;
			gridContent += "</td>";
		}
		gridContent += "</tr>";
	}
	...
	gridContent += "<br><table border='1px'  cellspacing='0px' width='auto' class='print'><tr>";
	gridContent +=  "<th align='left'>&nbsp;</th>";

	for(m=halfColumnsNum+1;m<parentgrid.getColumnsNum();m++){
		gridContent = gridContent+"<th align='left'>"+parentgrid.getHeaderCol(m)+"</th>";
	}
	gridContent +="</tr>";
	...
	for(n=start;n<(start+printRecordNumber);n++){
		...
		gridContent += "<tr>";
		for(i = halfColumnsNum;i<parentgrid.getColumnsNum();i++){
		
			gridContent += "<td align='left'>";
			gridContent += i==halfColumnsNum?n+1:parentgrid.cellByIndex(n, i).cell.innerHTML;
			gridContent += "</td>";
		}
		gridContent += "</tr>";
	}
