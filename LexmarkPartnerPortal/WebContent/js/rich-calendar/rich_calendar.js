...
//this.date.setDate(31);
	...
	// element which value is taken to initialize calendar and where calendar
	...
	// specifies the id of the text field associated with this calendar
	this.text_field_id = null;
...
//alert(obj.rc_object_code);
	switch (obj.rc_object_code) {
		case 'day':
//			alert(obj.day_num);
			// do nothing when the date before start date is selected
			if (cal.start_date != null && new Date(cur_year, cur_month, obj.day_num) < cal.start_date) {
				return;
			}
			// do nothing when the date behind end date is selected
			if (cal.end_date != null && new Date(cur_year, cur_month, obj.day_num) > cal.end_date) {
				return;
			}
			
			// remove 'rc_current' from the previous cell
			var tableRowNum = cal.table_obj.rows.length;
			var tableRow;
			var tableCell;
			var columnNum;
			for (var i = 0; i < tableRowNum; i ++) {
				tableRow = cal.table_obj.rows[i];
				columnNum = tableRow.cells.length;
				for (var j = 0; j < columnNum; j ++) {
					tableCell = tableRow.cells[j];
					if (tableCell.className.indexOf('rc_current') > -1) {
						RichCalendar.remove_class(tableCell, 'rc_current');
					}
				}
			}
			
			RichCalendar.add_class(obj, 'rc_current');
		...
		case 'prev_month':
			...
			break;
		case 'next_month':
			// determine number of days in next month
			cal.date.setDate(1);
			cal.date.setMonth(cur_month+1);
			var month_days = RichCalendar.get_month_days(cal.date);
...
var cur_day = cal.date.getDate();
	...
	switch (obj.rc_object_code) {
		case 'day':
			...
			if (obj.className.indexOf("rc_invalid_day") == -1) {
...
*/
	...
	if (this.show_time) {
		...
		this.hours_obj = this.createElement('SELECT', td);
		...
		for(i = 0;i < 24;i++){
			...
			this.hours_obj.options.add(new Option(optionText, i));
		}
...
//		this.colon_span.innerHTML = '&nbsp;:&nbsp;';
		...
		this.mins_obj = this.createElement('SELECT', td);
		...
		for(i = 0;i < 60;i++){
			...
			this.mins_obj.options.add(new Option(optionText, i));
		}
...
//		td.innerHTML = '&nbsp;';
	...
	// Select and Cancel
	this.button_tr = this.table_obj.insertRow(rows_num+3+(this.show_time?1:0));
	this.button_tr.className = 'rc_button_tr';
	var selectTd = this.button_tr.insertCell(0);
	selectTd.colSpan = 3;
	this.select_obj = this.createElement('BUTTON', selectTd);
	this.select_obj.rc_object_code = 'select';
	this.select_obj.calendar = this;
	this.select_obj.innerHTML = this.text('select');
	RichCalendar.attach_event(this.select_obj, 'click', RichCalendar.onclick);
	var emptyTd = this.button_tr.insertCell(1);
	emptyTd.innerHTML = '&nbsp;';
	var cancelTd = this.button_tr.insertCell(2);
	cancelTd.colSpan = 3;
	this.cancel_obj = this.createElement('BUTTON', cancelTd);
	this.cancel_obj.rc_object_code = 'cancel';
	this.cancel_obj.calendar = this;
	this.cancel_obj.innerHTML = this.text('cancel');
	RichCalendar.attach_event(this.cancel_obj, 'click', RichCalendar.onclick);

	// footer row

	this.footer_tr = this.table_obj.insertRow(rows_num+4+(this.show_time?1:0));
...
//alert('!: ' + cal_width + ' => ' + cal_height);
		...
		switch (aligns[1]) {
			...
			case "adj_bottom":
				...
				break;
			case "adjust":
				if (y - el.offsetHeight - cal_height >= 0) {
					y -= el.offsetHeight + cal_height;
				}
				break;
			//show the calendar at the bottom when the size is enough, else show the calendar at the top.
			case "adjust_bottom":
				if (document.body.offsetHeight - y - el.offsetHeight - cal_height < 0) {
					y -= el.offsetHeight + cal_height;
				}
				break;
			default:
...
if (this.start_date != null) {
	var start_date_year = this.start_date.getFullYear();
	var start_date_month = this.start_date.getMonth();
	var start_date_day = this.start_date.getDate();
}

// end_date
if (this.end_date != null) {
	var end_date_year = this.end_date.getFullYear();
	var end_date_month = this.end_date.getMonth();
	var end_date_day = this.end_date.getDate();
}
...
//				RichCalendar.detach_events(cell);
				...
				// gray out days before start date
				if (this.start_date != null) {
					if (new Date(cur_year, cur_month, day_num) < this.start_date) {
						RichCalendar.add_class(cell, "rc_invalid_day");
					}
				}
				
				// gray out days after end date
				if (this.end_date != null) {
					if (new Date(cur_year, cur_month, day_num) > this.end_date) {
						RichCalendar.add_class(cell, "rc_invalid_day");
					}
				}
...
//returns text_field_id 
RichCalendar.prototype.get_text_field_id = function() {
	return this.text_field_id;
}
...
RichCalendar.prototype.get_formatted_date = function(format, date) {
	...
	if (this.show_time && this.hours_obj && this.mins_obj) {
		...
		this.date.setMinutes(this.mins_obj.value);
	}
...
// calendar on close handler; returns true if operation successfully
