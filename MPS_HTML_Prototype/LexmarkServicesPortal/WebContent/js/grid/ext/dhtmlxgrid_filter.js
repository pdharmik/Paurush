...
/**
 * SVEN: 7/?/2011
 * Enable the combo_select filter
 * Change: 
 * 1.add method 'setCustomizeCombo'
 * 2.modify method '_loadComboOptins'
 * 3.z.enableFilteringMode(false)
 */


dhtmlXGridObject.prototype.setCustomizeCombo=function(comboValues,columnIndex){
	if(this.customizeCombobValues){
		this.customizeCombobValues[columnIndex] =  comboValues;
		if(comboValues.length < 5){
			this.autoHeightFlags[columnIndex] = [true,false];
		}
	}else{
		
		this.customizeCombobValues = [];
		this.autoHeightFlags = [];
		this.customizeCombobValues[columnIndex] =  comboValues;
		if(comboValues.length < 5){
			this.autoHeightFlags[columnIndex] = true;
		}
	}
}
	 ...
	 if(this.customizeCombobValues){
		 ...
		 if(l){
			 ...
			 for (var i=0; i<l.length; i++)
				 t.combo.addOption(l[i][0],l[i][1]);
			// t.combo.resetOldOption(oldIndex); //commented by sagar
			 t.combo.render(true); 
			 
		 }else{
			 l=this.collectValues(c);
			 t.combo.clearAll();
			 t.combo.render(false);
			 t.combo.addOption("","&nbsp;");
			 for (var i=0; i<l.length; i++)
