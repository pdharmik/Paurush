...
dhtmlXGridObject.prototype.enableHeaderMenu = function() {
	...
	this.attachEvent("onInit", function() {
		this.hdr.oncontextmenu = function(e) {
			return that._doHContClick(e || window.event);
		};
		{
...
dhtmlXGridObject.prototype._doHContClick = function(ev) {
	function mouseCoords(ev) {
		...
		;
		var d = ((_isIE) && (document.compatMode != "BackCompat")) ? document.documentElement
				: document.body;
		return {
	...
	if(this.location){
		this._showHContext(true, getposLeft(this.location), getposTop(this.location)+20);
	}else{
...
dhtmlXGridObject.prototype._createHContext = function() {
	...
	for ( var i; i < this.hdr.rows[1].cells.length; i++) {
		...
		if (c.firstChild && c.firstChild.tagName == "DIV")
			...
			var val = c.innerHTML;
		a
				.push("<div class='dhx_header_cmenu_item'><input type='checkbox' column='"
						+ true_ind
						+ "' len='"
						+ (c.colSpan || 1)
						+ "' checked='true' />" + val + "</div>");
...
dhtmlXGridObject.prototype._updateHContext = function() {
	for ( var i = 1; i < this._hContext.childNodes.length; i++) {
